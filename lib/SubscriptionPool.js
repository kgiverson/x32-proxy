...
import stripV6Brackets from './stripV6Brackets.js';
...
export default class SubscriptionPool {
  ...
  async sendUpstream(data, client) {
      ...
      this.pool.set(key, {
        ...
        socketPromise: new Promise((resolve, reject) => {
          const socket = dgram.createSocket(
            net.isIPv6(stripV6Brackets(argv.target)) ? 'udp6' : 'udp4'
          );
    ...
    socket.send(data, argv.targetPort, stripV6Brackets(argv.target));
    return true;
  }
}